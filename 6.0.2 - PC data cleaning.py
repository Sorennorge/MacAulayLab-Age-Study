...
Folder2 = "Data/Panther/Protein classes"
...
File6 = "Protein classes overview.xlsx"
...
df_MF = pd.DataFrame(columns=['Gene name','PC'])
...
for key in MF_dict:
    MF_dict[key]['PC'] = list(set(MF_dict[key]['PC']))
        ...
        if key not in overview_dict:
            ...
            if any(x in uni_list_1 for x in uni_list_2):
                ...
                if key == key_test:
                    print("Catch 2")
                """
                for item in MF_dict[diff_dict[key]]['PC']:
                    overview_dict[key]['PC'].append(item)
...
for key in Handle_dict_one:
    overview_dict[key] = {}
    overview_dict[key]['Gene'] = diff_dict[key]
    overview_dict[key]['PC'] = ['Unclassified']
    overview_dict[key]['Uniprot'] = []

### transform overview dict to dataframe and save dataframe ###

for key in overview_dict:
    if len(overview_dict[key]['PC']) > 1:
        overview_dict[key]['PC'] = "{}".format(";".join(overview_dict[key]['PC']))
    else:
        overview_dict[key]['PC'] = "{}".format(overview_dict[key]['PC'][0])
